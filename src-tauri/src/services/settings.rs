...
impl SettingsManager {
    ...
    pub fn set_skhd_variant_setting(&self, setting: SkhdVariantSetting) -> io::Result<()> {
        ...
        settings.skhd_variant = setting;
        drop(settings); // Release lock before save

        self.save_settings(&self.settings.lock().unwrap())
    }
...
mod tests {
    ...
    fn test_settings_manager_save_and_load() {
        ...
        let loaded_settings = SettingsManager::load_settings(&temp_dir.path().to_path_buf());
        assert!(matches!(loaded_settings.skhd_variant, SkhdVariantSetting::Zig));
    }
