...
- **Config Paths**: Both variants check `$XDG_CONFIG_HOME/skhd/skhdrc`, `~/.config/skhd/skhdrc`, then `~/.skhdrc`.
  - Implementation: `utils/path.rs` owns the canonical config search order
...
- **Parser Extensions**: `parser/` uses PEST grammar with skhd.zig directive support
  ...
  - Typed support: `.alias`, `.define` (group and command forms), `.path`, `.shell`, `.blacklist`, `.load`; other directives are preserved as generic read-only nodes
