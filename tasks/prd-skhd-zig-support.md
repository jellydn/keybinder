...
**Description:** As a user with an XDG-style setup or a skhd.zig-only install, I want Keybinder to find my skhdrc the same way skhd.zig does.

**Acceptance Criteria:**
- [ ] `service_manager::get_config_path` (or equivalent) checks paths in this order when variant is skhd.zig: `$XDG_CONFIG_HOME/skhd/skhdrc`, `~/.config/skhd/skhdrc`, `~/.skhdrc`.
- [ ] When variant is original skhd, behavior is unchanged: `~/.config/skhd/skhdrc`, `~/.skhdrc`.
...
- **FR-3:** When `skhd_variant = auto`, app MUST resolve to a concrete variant using FR-2 and prefer a *running* variant over a merely-installed one.
...
- **FR-7:** Config-path resolution for skhd.zig MUST include `$XDG_CONFIG_HOME/skhd/skhdrc` first.
...
- Migrating user *config files* between variants (no syntax-stripping or up-converting). Configs are presented as-is. **Service migration** (stop original / install zig / start) IS in scope — see US-012.
